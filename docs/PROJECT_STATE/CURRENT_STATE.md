...
- **Visual dashboard, verified running**: execution map derived from real persisted execution (never a static diagram), failure localization, component diagnostics, MCP view, agent communication, plan evolution, system-wide component health. Served at `http://127.0.0.1:8011/dashboard` (port 8000 on this machine turned out to be a WSL relay, not this server -- see `BLOCKERS.md` B15).
